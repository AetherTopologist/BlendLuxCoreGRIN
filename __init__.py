...
# Force Blender to use ONLY a local custom .whl from pyluxcore_custom/
USE_ONLY_CUSTOM_WHEEL = True
...
if am_in_extension:
        ...
        with open(info_file, 'r') as f:
            for line in f.readlines():
                ...
                if line.startswith('#'):
                    ...
                    old_wheel_hash = line.split(':', 1)[1].strip()
                elif line.startswith('pyluxcore-version:'):
                    old_pyluxcore_version = line.split(':', 1)[1].strip()

        print(f"[BLC] Cached wheel hash: {old_wheel_hash}")
        print(f"[BLC] Cached pyluxcore version: {old_pyluxcore_version}")
    ...
    def download_pyluxcore():
        if USE_ONLY_CUSTOM_WHEEL:
            if not blc_wheel_path:
                raise RuntimeError("[BLC] USE_ONLY_CUSTOM_WHEEL is enabled, but no valid custom .whl was found in 'pyluxcore_custom/'.")
            print("[BLC] Skipping download — using ONLY custom wheel as specified.")
            wheel_hash = base64.urlsafe_b64encode(hashlib.sha256(open(blc_wheel_path, 'rb').read()).digest()).decode('latin1').rstrip('=')
            old_wheel_hash, _ = _get_installation_info()
            if wheel_hash == old_wheel_hash:
                print("[BLC] Custom wheel already installed (hash matched). Skipping reinstallation.")
                return 2
            print("[BLC] Installing custom wheel (forced override mode).")
            command = [
                sys.executable, '-m', 'pip', 'download',
                blc_wheel_path, '-d', wheel_dl_folder
            ]
            try:
                _backup_wheels()
                ...
                raise RuntimeError(f"[BLC] Failed to install custom wheel in override mode: {str(e)}") from None

        # If override was successful, skip all other logic
